...
const STYLE_ID = "cig-output-gpu-touch-scrub-style";
...
function videoForTarget(target) {
    ...
    if (target.classList.contains("cig-native-fs-shell")) {
        const video = target.querySelector("video.ovg-inline-video[data-cig-native-fullscreen=\"1\"]");
        return video instanceof HTMLVideoElement ? video : null;
    }
    return null;
}
...
function attach(target) {
    ...
    target.addEventListener("pointerup", event => finish(event, false), { passive:false, capture:true });
    target.addEventListener("pointercancel", event => finish(event, true), { passive:false, capture:true });
...
function scan(root) {
    ...
    if (root.classList.contains("ovg-gpu-hit") || root.classList.contains("cig-native-fs-shell")) attach(root);
    root.querySelectorAll?.(".ovg-gpu-hit,.cig-native-fs-shell[data-cig-native-shell=\"1\"]").forEach(attach);
}
