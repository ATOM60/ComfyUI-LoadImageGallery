...
/* The old GPU module still creates its popover; hide it while the CPU-style GPU player owns controls. */
...
function cleanupRemoved(root){if(!(root instanceof Element))return;if(root instanceof HTMLVideoElement&&root.classList.contains("ovg-inline-video"))cleanupVideo(root);root.querySelectorAll?.("video.ovg-inline-video").forEach(cleanupVideo);}
